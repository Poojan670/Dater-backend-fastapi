...
def create_userdetail(request: schemas.UserDetails, db: Session = Depends(get_db),
...
def get_userdetail(db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
    user_details = db.query(models.UserDetails).all()
    return user_details


@router.get('/{id}', status_code=200, response_model=schemas.UserDetails)
async def show_userdetail(id, response: Response, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
...
async def update(id, request: schemas.UserDetails, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
    object = db.query(models.UserDetails).filter(
        models.UserDetails.id == id)
    if not object.first():
        raise HTTPException(
